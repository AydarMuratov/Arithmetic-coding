...
// Класс для работы с битовыми потоками
class BitStream {
public:
...
// Класс для работы с арифметическим кодированием
...
public:
    ...
    void encode(string inputFilename, string outputFilename) {
        ...
        while ((symbol = inputFile.get()) != EOF) {
            int index = alphabet.symbolToIndex(symbol);
            ...
            while (true) {
                ...
                } else if (lower >= 0.25 && upper
                pendingBits.push_back(upper >= 0.75);
                upper = 2 * (upper - 0.5);
                lower = 2 * (lower - 0.5);
            } else {
                break;
